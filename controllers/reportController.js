...
const renderHistoryDashboard = async (req, res) => {
  try {
    ...
    // FUSIÓN: Lógica de seguridad para filtrar pacientes
    if (role === 'Terapeuta') {
        ...
        // GestorHistorial (Renan), Admin, Monitorista: Ven TODOS los pacientes
        const result = await db.query(`
            SELECT p.id_paciente, u.nombre, u.apellido, u.foto_perfil, u.email,
                   p.estado_tratamiento, p.fecha_inicio_tratamiento,
...
const renderPatientHistoryView = async (req, res) => {
  try {
    ...
    // SEGURIDAD: Verificar acceso si es terapeuta (Renan pasa directo)
    if (role === 'Terapeuta') {
...
const generateProgressReport = async (req, res) => {
    ...
    try {
        ...
        // Buscar ID Terapeuta (Si es Renan, usamos el asignado al paciente)
...
const updateSessionNote = async (req, res) => {
    ...
    try {
        ...
        // Permitir a Terapeutas, Gestores y Admin editar notas
...
// ====================================================================
// 6. ANALYTICS DASHBOARD (TU CÓDIGO RESTAURADO)
// ====================================================================
...
module.exports = {
  ...
  renderAnalyticsDashboard, // <--- Aquí está tu Analytics restaurado
