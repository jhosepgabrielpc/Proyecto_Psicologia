...
// ------------------------------------------------------------
// Helper: cargar contactos + lista de usuarios buscables
// ------------------------------------------------------------
async function loadContactsAndUsers(userId, userRole) {
    // A. CONTACTOS (gente con la que ya hablé)
    const contactsQuery = `
        ...
        JOIN Mensajes_Seguros m 
          ON (m.id_remitente = u.id_usuario AND m.id_destinatario = $1) 
    ...
    let contacts = contactsRes.rows.sort(
        (a, b) => new Date(b.fecha_envio) - new Date(a.fecha_envio)
    );

    // B. LISTA GLOBAL DE USUARIOS (para iniciar nuevas conversaciones)
    // Filtro por rol según quién soy:
    // - Paciente: solo ve Terapeutas y Admin
    // - Terapeuta: ve Pacientes, Terapeutas y Admin
    // - Admin: ve todos
    let baseWhere = 'u.id_usuario != $1 AND u.estado = true';
    if (userRole === 'Paciente') {
        baseWhere += " AND r.nombre_rol IN ('Terapeuta','Admin')";
    } else if (userRole === 'Terapeuta') {
        baseWhere += " AND r.nombre_rol IN ('Paciente','Terapeuta','Admin')";
    } // Admin no tiene restricción adicional

    const allUsersQuery = `
        SELECT u.id_usuario, u.nombre, u.apellido, r.nombre_rol, u.foto_perfil, u.email
        FROM Usuarios u 
        JOIN Roles r ON u.id_rol = r.id_rol 
        WHERE ${baseWhere}
...
const getChatInterface = async (req, res) => {
    ...
    const targetId = req.query.chat ? parseInt(req.query.chat, 10) : null;
    const filterRole = req.query.role || null; // para futuros filtros por rol en UI

    try {
        // A. CONTACTOS + LISTA GLOBAL
        const data = await loadContactsAndUsers(userId, userRole);
        let contacts = data.contacts;
        let allUsers = data.allUsers;

        // Filtro opcional por rol (para futuros dropdowns en el front)
        if (filterRole) {
            contacts = contacts.filter(c => c.nombre_rol === filterRole);
            allUsers = allUsers.filter(u => u.nombre_rol === filterRole);
        }

        // B. LOGICA DE CHAT ACTIVO
        let activeChat = null;
        let messages = [];
        ...
        if (targetId) {
            ...
            if (targetRes.rows.length > 0) {
                ...
                const existsInContacts = contacts.find(
                    c => c.id_usuario === targetId
                );
...
// ====================================================================
// 1.bis. API JSON PARA CONTACTOS (para futuros filtros/buscador) 🔍
// ====================================================================
const getContactsJson = async (req, res) => {
    const userId = req.session.user.id_usuario;
    const userRole = req.session.user.nombre_rol;
    ...
    try {
        ...
        console.error('Error obteniendo contactos JSON:', error);
        res.status(500).json({ ok: false, contacts: [], allUsers: [] });
    }
};
...
const sendMessage = async (req, res) => {
    const { receiverId, content } = req.body; // El frontend idealmente manda 'receiverId' y 'content'
    ...
    try {
        if (!texto || !destId) {
            return res.status(400).json({
                success: false,
                error: 'Mensaje vacío o destinatario inválido'
            });
        }
        ...
        res.status(500).json({
            success: false,
            error: 'Error interno de mensajería'
        });
    }
};
